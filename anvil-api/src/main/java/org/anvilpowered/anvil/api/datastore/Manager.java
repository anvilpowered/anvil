 ...
 * </p>
 * <ul>
 *     <li>{@code CompletableFuture<Component> create(UUID userUUID);}</li>
 *     <li>{@code CompletableFuture<Component> invite(UUID userUUID, UUID targetUserUUID);}</li>
 *     <li>{@code CompletableFuture<Component> kick(UUID userUUID, UUID targetUserUUID);}</li>
 *     <li>{@code CompletableFuture<List<Component>> list(String query);}</li>
 * </ul>
 * <p>
 * {@code Component} is the base return type for the methods in a {@link Manager}.
 * To build these results use {@link TextService.Builder}.
 * </p>
 * <p>
 * All methods (with some exceptions) in {@link Manager} should return a form of {@code Component}
 * to be displayed directly to the end user. Normally, the return type, {@code Component}, is wrapped in
 * a {@link java.util.concurrent.CompletableFuture} in order to keep the main game thread
 * free from IO. It is sometimes necessary to further wrap the {@code Component} in a {@link java.util.List}
