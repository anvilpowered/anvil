...
fun <T : Any> Registry.serializeDefault(
  key: Key<T>,
  json: Json = Json,
): String = key.serialize(getDefault(key), json)

fun <T : Any> Registry.serializeStrict(
  key: SimpleKey<T>,
  json: Json = Json,
): String? = getStrict(key)?.let { key.serialize(it, json) }

fun <T : Any> Registry.serialize(
  key: SimpleKey<T>,
  json: Json = Json,
): String = key.serialize(get(key), json)

fun <E : Any> Registry.serializeStrict(
  key: ListKey<E>,
  json: Json = Json,
): String? = getStrict(key)?.let { key.serialize(it, json) }

fun <E : Any> Registry.serialize(
  key: ListKey<E>,
  json: Json = Json,
): String = key.serialize(get(key), json)

fun <K : Any, V : Any> Registry.serializeStrict(
  key: MapKey<K, V>,
  json: Json = Json,
): String? = getStrict(key)?.let { key.serialize(it, json) }

fun <K : Any, V : Any> Registry.serialize(
  key: MapKey<K, V>,
  json: Json = Json,
): String = key.serialize(get(key), json)

fun Registry.serialize(
  key: Key<*>,
  json: Json = Json,
): String =
  when (key) {
