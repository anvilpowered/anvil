...
interface Registry {
  ...
  fun <E : Any> getDefault(
    key: ListKey<E>,
    index: Int,
  ): E

  fun <K : Any, V : Any> getDefault(
    key: MapKey<K, V>,
    mapKey: K,
  ): V
  ...
  operator fun <E : Any> get(
    key: ListKey<E>,
    index: Int,
  ): E = getStrict(key, index) ?: getDefault(key, index)

  operator fun <K : Any, V : Any> get(key: MapKey<K, V>): Map<K, V> = getStrict(key) ?: getDefault(key)

  operator fun <K : Any, V : Any> get(
    key: MapKey<K, V>,
    mapKey: K,
  ): V = getStrict(key, mapKey) ?: getDefault(key, mapKey)
