...
class NukkitLocationService : CommonLocationService() {
    ...
    override fun getWorldName(userUUID: UUID): Optional<String> = userService[userUUID].map { it.getLevel().name }
    override fun getWorldName(userName: String): Optional<String> = userService[userName].map { it.getLevel().name }
