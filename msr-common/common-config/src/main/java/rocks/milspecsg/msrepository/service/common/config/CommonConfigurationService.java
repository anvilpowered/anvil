...
public abstract class CommonConfigurationService implements ConfigurationService {
...
//    protected Map<Integer, Map<? extends Predicate<? extends Map<?, ?>>, ? extends Function<? extends Map<?, ?>, ? extends Map<?, ?>>>> mapVerificationMap;
    ...
    protected abstract void initNodeTypeMap();

    protected abstract void initVerificationMaps();

    protected abstract void initDefaultMaps();
    ...
    public void addConfigLoadedListener(ConfigLoadedListener configLoadedListener) {
        configLoadedListeners.add(configLoadedListener);
    }

    @Override
    public void removeConfigLoadedListener(ConfigLoadedListener configLoadedListener) {
        configLoadedListeners.remove(configLoadedListener);
    }
    ...
    private <T> void saveConfigValue(Integer nodeKey, CommentedConfigurationNode node, TypeToken<T> typeToken) {
        getConfig(nodeKey, typeToken).ifPresent(node::setValue);
    }

    private <T> void saveDefaultValue(Integer nodeKey, CommentedConfigurationNode node, TypeToken<T> typeToken) {
     ...
     */
    private  <T> T initConfigValue(Integer nodeKey, CommentedConfigurationNode node, TypeToken<? extends T> typeToken, boolean[] modified) {
    ...
    protected <T> T getAssertedType(int key, TypeToken<T> typeToken, Function<Integer, ?> getter) {
        ...
        if (a && b) {
            ...
            System.err.println("Type in map: " + nodeTypeMap.get(key));
            System.err.println("Asserted type: " + typeToken);
