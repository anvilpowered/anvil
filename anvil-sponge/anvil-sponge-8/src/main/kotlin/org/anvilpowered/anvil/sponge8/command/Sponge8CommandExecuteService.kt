...
class Sponge8CommandExecuteService : CommandExecuteService {
  ...
  private fun executeDirect(command: String) {
    Sponge.getServer().commandManager.process(Sponge.getSystemSubject(), command)
  }
  ...
  override fun execute(command: String) {
    if (Sponge.getServer().onMainThread()) {
      ...
      Sponge.getAsyncScheduler().submit(Task.builder().execute { _ -> executeDirect(command) }.plugin(pluginContainer).build())
    }
  }
}
