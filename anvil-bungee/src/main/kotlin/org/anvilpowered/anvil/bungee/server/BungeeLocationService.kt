...
class BungeeLocationService : CommonLocationService() {
    ...
    private fun getServer(user: Optional<ProxiedPlayer>): Optional<BungeeBackendServer> = user.map { it.server }.map {
        BungeeBackendServer(it.info, userService)
    }
    ...
    override fun getServerForName(serverName: String): Optional<BungeeBackendServer> =
        Optional.ofNullable(ProxyServer.getInstance().getServerInfo(serverName))
            .map { BungeeBackendServer(it, userService) }

    override fun getServers(): List<BungeeBackendServer> =
        ProxyServer.getInstance().servers.values.stream().map { BungeeBackendServer(it, userService) }.toList()
}
